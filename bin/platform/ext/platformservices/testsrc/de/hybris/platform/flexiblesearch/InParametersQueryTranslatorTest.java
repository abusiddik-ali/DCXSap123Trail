...
public class InParametersQueryTranslatorTest extends ServicelayerBaseTest
{
	...
	@Test
	public void shouldAnalyzeInClauseWithLiteralsWithSQLInjection()
	{
		// given
		final InParametersQueryTranslator translator = createQueryTranslator(3);
		final Map<String, List<String>> overMaxParams = ImmutableMap.of("names",
				ImmutableList.of("foo", "bar", "baz' UNION SELECT 'inject' FROM DUAL UNION 'bat", "bazinga"));
		...
		// then
		assertThat(withQuery).contains("'baz'' UNION SELECT ''inject'' FROM DUAL UNION ''bat'");
	}

	@Test
	public void shouldAnalyzeAndGenerateWithForInClauseWithSQLInjection()
	{
		// given
		final InParametersQueryTranslator translator = createQueryTranslator(3);
		final Map<String, List<String>> overMaxParams = ImmutableMap.of("names",
				ImmutableList.of("foo","baz' UNION SELECT 'inject' FROM DUAL UNION 'bat", "bar", "baz", "bazinga"));
