					...
					<fieldset>
						...
						<table id="allTestSuites">
							...
							<c:forEach items="${testSuites}" var="testClass">
                                <c:set var="testClassName" value="${fn:escapeXml(testClass.name)}"/>
                                <c:set var="testClassSimpleName" value="${fn:escapeXml(testClass.simpleName)}"/>
								<tr>
									<td>
										<img src="<c:url value="/static/img/expand-small-silver.png"  />"
											 id="test-cases-toggle_${testClassName}"
											 class="test-cases-toggle" data-testSuite="${testClassName}"
											 data-contentContainerId="testCasesFor_${testClassSimpleName}"/>

										<form:checkbox path="testSuites" class="testSuite" id="testClass_${testClassName}"
													   value="${testClassName}"
													   data-contentContainerId="testCasesFor_${testClassSimpleName}"/>
										<form:label path="testSuites" for="testClass_${testClassName}"
													data-contentContainerId="testCasesFor_${testClassSimpleName}">
											<c:out value="${testClassName}" />
										</form:label>
							<span id="testCasesFor_${testClassSimpleName}">
								<!-- Ajax loaded content -->
							</span>
									</td>
								</tr>
					...
					<fieldset>
						...
						<table id="filteredTestSuites">
							...
							<c:forEach items="${filteredTestSuites}" var="testClass">
                                <c:set var="testClassName" value="${testClass.name}"/>
                                <c:set var="testClassSimpleName" value="${testClass.simpleName}"/>
								<tr>
									<td>
										<img src="<c:url value="/static/img/expand-small-silver.png"  />"
											 id="filtered-test-cases-toggle_${testClassName}"
											 class="test-cases-toggle" data-testSuite="${testClassName}"
											 data-contentContainerId="testCasesFor_${testClassSimpleName}"/>

										<form:checkbox path="testSuites" class="filteredTestSuite" id="filtered-testClass_${testClassName}"
													   value="${testClass.name}"
													   data-contentContainerId="testCasesFor_${testClassSimpleName}"/>
										<form:label path="testSuites" for="filtered-testClass_${testClassName}"
													data-contentContainerId="testCasesFor_${testClassSimpleName}">
											<c:out value="${testClassName}" />
										</form:label>
							<span id="filteredtestCasesFor_${testClassSimpleName}">
								<!-- Ajax loaded content -->
							</span>
									</td>
								</tr>
