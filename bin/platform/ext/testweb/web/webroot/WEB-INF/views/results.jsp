		...
		<table id="test-results-table">
			...
			<c:forEach var="testResult" items="${testResults}">
				<c:forEach var="tesSuiteResult" items="${testResult.testResults}">
                    ...
                    <c:set var="testSuiteClassSimpleName" value="${fn:escapeXml(testSuiteData.testSuiteClass.simpleName)}"/>
					...
					<tr>
						<td><img src="<c:url value="/static/img/expand-small-silver.png" />"
									...
									data-testCasesClass="${testSuiteClassSimpleName}_testCases"/></td>
						<td>
							<a class="re-run-test"
								title="re-run test suite"
								href="<c:url value="/run/testsuites?testSuites=${testSuiteData.testSuiteClass.name}"/>">
								${fn:escapeXml(testSuiteData.testSuiteClass.name)}
							</a>
						</td>
					...
					<c:forEach var="testCaseData" items="${testSuiteData.results}">
						<tr class="${testSuiteClassSimpleName}_testCases testCasesContainer">
							<td colspan="2">
								<spring:eval
								...
								${fn:escapeXml(testCaseData.testCaseMethod)}
								</a>
							</td>
							<td><testweb:duration value="${testCaseData.elapsedTime}" format="m 'm' s 's' SS 'ms'"/></td>
							<td data-error="${fn:escapeXml(testCaseData.failure)}"
								 data-stackTrace="${fn:escapeXml(testCaseData.failure.trace)}">
								<c:choose>
